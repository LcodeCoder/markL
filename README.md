...
- **左侧**：最左边图标栏（目录 / 结构 / 搜索），右边是品牌、打开文件夹、文件树、标题结构或工作区搜索、打开历史。文档可钉在顶部。目录栏可拖右边调整宽度。
- **右侧上方**：当前文件名、未保存圆点、完整路径、新建 / 打开 / 保存、设置、检查更新。
- **中间**：Vditor 即时渲染编辑区，正文铺满可用宽度。
- **底部**：保存状态、即时渲染 / 源码切换、编码、字数；有选区时同时显示选区字数。
...
打开时识别 UTF-8（含 BOM）、UTF-16 和常见 GBK / GB2312；**保存一律写成 UTF-8**。支持扩展名：`.md`、`.markdown`、`.txt`。把这些文件拖进窗口也会打开。已保存的文档默认约 1.6 秒自动保存，可在设置里关掉。
...
- `Aa` 区分大小写；`W` 全词；`.*` 正则；可替换当前或全部替换
- 侧栏搜索可对整个文件夹全部替换（会立刻写盘，先确认）
...
- 颜色跟当前浅色 / 深色 / 护眼主题走
- 公式样式打进文件里，不走 CDN，离线也能看
- 菜单「文件 → 导出 PDF」可再出一份 A4 PDF
- 不依赖 MarkL 本身，发给别人用浏览器就能看

### 10. 主题、字体与设置

菜单「主题」可切换浅色、青雾、护眼、深色、墨夜、海暮。菜单「字体」可选微软雅黑、宋体、楷体等，以及四种字号。顶栏「设置」在「帮助」旁边，点开会弹出设置窗口：自动保存、跟随系统浅色/深色、新文档默认源码、行号都是开关。也可 `Ctrl+,`。选择会记在本地。
...
3. **即时渲染 / Markdown 源码**
4. **UTF-8**
5. **字数 · 字符数 · 行 · 段**（中文按字、英文按词；有选区时再显示选区字数）
...
| 菜单 | 命令                                                                                                                               |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 文件 | 新建文档、打开文件、打开文件夹、快速打开、保存、另存为、从模板新建、导出 HTML / PDF、打印、本地历史、退出 MarkL                    |
| 编辑 | 撤销 / 重做、剪切 / 复制 / 粘贴 / 删除、全选、查找、替换、在文件夹中查找、格式化代码块、整理中文排版、插入已有图片、清理未使用图片 |
| 视图 | 切换即时渲染 / 源码、显示/隐藏目录栏、重新加载、缩放、全屏                                                                         |
| 主题 | 浅色、青雾、护眼、深色、墨夜、海暮                                                                                                 |
| 字体 | 正文字体、字号                                                                                                                     |
| 设置 | 打开设置弹窗                                                                                                                       |
...
| [electron-updater](https://www.electron.build/auto-update) | 安装版应用内下载更新               |
...
- 启动后会检查 GitHub Release；有新版本会提示。NSIS 安装版可在应用内下载并安装，绿色版仍打开浏览器下载。也可点顶栏或菜单「帮助 → 检查更新」。
